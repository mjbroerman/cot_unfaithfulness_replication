...
# Get the BBH data directory
bbh_dir = Path(user_cache_dir("inspect_evals")) / "cot-unfaithfulness" / "data" / "bbh"

# Collect counts for each category
results = []

for category_dir in sorted(bbh_dir.iterdir()):
    if category_dir.is_dir():
        json_file = category_dir / "val_data.json"
        if json_file.exists():
...
So we see the 300 after reserving 30, and the cases where fewer existed. There aren't unique ids on the subsampled problems. So we assign them as the rowid in the alphabetic order of the directories.

We will also replicate with BBH and BBEH found in `inspect_evals`

#### BBQ

```{python}
# Get the BBQ data directory
bbq_dir = Path(user_cache_dir("inspect_evals")) / "cot-unfaithfulness" / "data" / "bbq"

# Load the data.json file
json_file = bbq_dir / "data.json"
with open(json_file, 'r') as f:
    data = json.load(f)
...
print(f"Total samples: {category_counts.sum()}")
```

We will plan to replication the results on these sets, and for 
